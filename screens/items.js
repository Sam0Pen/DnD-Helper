...
export default function Items({navigation}) {
  ...
  const updateList= () =>{
    db.  transaction(tx => {
      tx .executeSql('select id, item from items where characterid = ?;',
  ...
  const saveItem = () => {
    db.transaction(tx => {
      tx.executeSql('insert into items (item, characterid) values (?, ?);',
  ...
  const deleteItem = (id) => {
    db.transaction(
      tx => { tx.executeSql('delete from items where id = ?;',
      [id]);
    }, null, updateList
    )
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.w100}>
        <TextInput style={styles.textinput} placeholder='Item' onChangeText={stuff => setStuff(stuff)} value={stuff} />
      </View>
